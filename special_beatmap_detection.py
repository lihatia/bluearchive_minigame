...
image_save = "./special"
wname = "img"
left_tap_zone = [(223, 377), (520, 408)]
right_tap_zone = [(753, 377), (1060, 412)]
...
frame_image_left_none = cv2.imread(image_save + "/image0.jpg", cv2.IMREAD_COLOR)
frame_image_left_tap = cv2.imread(image_save + "/image188.jpg", cv2.IMREAD_COLOR)
frame_image_left_press = cv2.imread(image_save + "/image1910.jpg", cv2.IMREAD_COLOR)

frame_image_right_none = cv2.imread(image_save + "/image0.jpg", cv2.IMREAD_COLOR)
frame_image_right_tap = cv2.imread(image_save + "/image202.jpg", cv2.IMREAD_COLOR)
frame_image_right_press = cv2.imread(image_save + "/image1935.jpg", cv2.IMREAD_COLOR)
...
mean_left_press = mean_pixel(left_tap_zone, frame_image_left_press)
...
mean_right_press = mean_pixel(right_tap_zone, frame_image_right_press)
...
mean_left_tap_zone = [mean_left_tap_none, mean_left_tap, mean_left_press]
mean_right_tap_zone = [mean_right_tap_none, mean_right_tap, mean_right_press]
...
def action_determine(img):
    ...
    return action, left_tap_flag, right_tap_flag
...
for i in range(int(frame_count)):
    ...
    _, img = cap.read()
    action, left_tap_flag, right_tap_flag = action_determine(img)
...
for i in range(int(frame_count)):
    if action_list[i][0] + action_list[i][1] != 0:
        try:
            if action_list[i][0] == 4:
                if action_list[i + 1][0] == 5:
                    action_list[i][0] = 1
                    action_list[i + 1][0] = 0

            if action_list[i][1] == 4:
                if action_list[i + 1][1] == 5:
                    action_list[i][1] = 1
                    action_list[i + 1][1] = 0

            if action_list[i + 1][0] == 4:
                action_list[i][0] = 4
                action_list[i + 1][0] = 2

            if action_list[i + 1][1] == 4:
                action_list[i][1] = 4
                action_list[i + 1][1] = 2

            if action_list[i][0]==1:
                if action_list[i+1][0]==1:
                    action_list[i][0]=0

            if action_list[i][1]==1:
                if action_list[i+1][1]==1:
                    action_list[i][1]=0
        except:
            print(i)
            print(len(action_list))

final_frame_time_list=[]
final_action_list=[]
for i in range(int(frame_count)):
    if action_list[i][0] + action_list[i][1] != 0:
        final_frame_time_list.append(frame_time_list[i])
        final_action_list.append(action_list[i])


with open("sp_frame_time.pkl", "wb") as f:
    pickle.dump(final_frame_time_list, f)
with open("sp_beatmap.pkl", "wb") as f:
    pickle.dump(final_action_list, f)

with open("sp_action.csv", "w", encoding='utf8', newline='') as f:
    output_flie = csv.writer(f)
    output_flie.writerows(action_list)
